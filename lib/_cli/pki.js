...
function runOpenSslCommand (description, commandArgs, opensslBin, verbosity) {
  ...
  var command = spawnSync(opensslBin, commandArgs)
...
function generatePrivateKey (fileName, opensslBin, verbosity) {
  runOpenSslCommand('Saving private key file to ' + fileName, ['genrsa',
    '-out', fileName, PUBLIC_KEY_BITS], opensslBin, verbosity)
...
function generateCsr (privateKeyFileName, csrFileName, commonName,
  ...
  fs.writeFileSync(tmpOpenSslConfigFile.name, requestConfig.join('\n'))
  ...
  try {
    runOpenSslCommand('Saving csr file to ' + csrFileName, ['req',
      ...
      '-config', tmpOpenSslConfigFile.name], command.opensslbin, verbosity)
  } finally {
