...
function findOpenSslBin (opensslBin) {
  if (opensslBin) {
    if (!fs.existsSync(opensslBin)) {
      util.throwError('Unable to find openssl at: ' + opensslBin)
    }
  } else {
    ...
    if (!opensslBin) {
      util.throwError('Unable to find openssl from system path')
    }
  }
  return opensslBin
}
...
function runOpenSslCommandWithEnvVars (description, commandArgs, opensslBin,
  ...
  if (command.status !== 0) {
    if (verbosity < 2 && command.stderr) {
      util.logError(command.stderr, '', 'ERROR LOG')
    }
    ...
    util.throwError(errorMessage, description)
  }
