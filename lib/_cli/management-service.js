...
function httpGet (description, currentRedirects, requestOptions, queryString,
  ...
  var request = https.get(requestOptions, function (response) {
    ...
    if (response.statusCode === 302) {
      if (currentRedirects === HTTP_MAX_REDIRECTS) {
        util.throwError('Reached maximum redirects for request to ' +
          requestUrl, description)
      } else {
        var redirectLocation = response.headers.location
        if (!redirectLocation) {
          util.throwError('Redirect with no location specified for ' +
            requestUrl, description)
        }
        ...
        if ((parsedRedirectLocation.hostname &&
          ...
          util.throwError('Redirect to different host or port not supported. ' +
    ...
    response.on('end', function () {
      if (response.statusCode >= 200 && response.statusCode <= 299) {
        ...
        util.throwError('Request to ' + requestUrl +
  ...
  request.on('error', function (error) {
    util.throwError('Error processing request to ' + requestUrl + ': ' + error,
      description)
  })
}
  ...
  httpGet(description, 0, requestOptions,
    queryString, verbosity, function (response) {
      ...
      if (responseStatusDelimiter < 0) {
        util.throwError("Did not find ':' status delimiter in response body " +
      ...
      if (status !== 'OK') {
        util.throwError('Request to ' + requestUrl + ' failed with status ' +
