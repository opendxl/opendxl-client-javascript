...
function runOpenSslCommand (description, commandArgs, verbosity) {
  ...
  var command = spawnSync('openssl', commandArgs)
  if (command.status !== 0) {
...
function generatePrivateKey (fileName, verbosity) {
...
function generateCsr (privateKeyFileName, csrFileName, commonName, verbosity) {
  runOpenSslCommand('Saving csr file to ' + csrFileName, ['req',
    '-out', csrFileName, '-key', privateKeyFileName, '-new',
    '-subj', '/CN=' + commonName], verbosity)
}
...
module.exports = {
  ...
  generatePrivateKeyAndCsr: function (configDir, filePrefix, commonName,
    ...
    generatePrivateKey(privateKeyFileName, verbosity)
    var csrFileName = path.join(configDir, filePrefix + '.csr')
    generateCsr(privateKeyFileName, csrFileName, commonName, verbosity)
