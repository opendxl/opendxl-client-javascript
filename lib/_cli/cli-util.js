...
module.exports = {
  ...
  getValueFromPrompt: function (title, confirm, callback) {
    let fullTitle = title.indexOf('Confirm') === 0 ? title : 'Enter ' + title
    fullTitle += ':'
    read({
      prompt: fullTitle,
      silent: true
  ...
  pullHostInfoFromCommand: function (hostname, command) {
    const hostInfo = { hostname, user: '', password: '' }
    const hostInfoKeys = ['port', 'user', 'password', 'truststore']
