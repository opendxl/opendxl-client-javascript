...
function buildQueryString (obj) {
  const query = Object.keys(obj).reduce(function (acc, key) {
...
function httpGet (description, currentRedirects, requestOptions, verbosity,
  responseCallback) {
  const requestUrl = toUrlString(requestOptions)
  let responseBody = ''
  ...
  const request = https.get(requestOptions, function (response) {
    ...
    if (response.statusCode === 302) {
      if (currentRedirects === HTTP_MAX_REDIRECTS) {
        ...
        const parsedRedirectLocation = new URL(redirectLocation, `${requestOptions.protocol}//${requestOptions.hostname}:${requestOptions.port}`)
        ...
        requestOptions.path = parsedRedirectLocation.pathname + parsedRedirectLocation.search
        httpGet(description, currentRedirects + 1, requestOptions, verbosity, responseCallback)
        response.resume()
        return
      }
    }
  ...
  httpGet(description, 0, requestOptions, verbosity,
    function (error, response) {
      ...
      if (response) {
        ...
        const responseStatusDelimiter = response.indexOf(':')
        if (responseStatusDelimiter < 0) {
          ...
          const status = response.substring(0, responseStatusDelimiter).trim()
          const responseDetail = response.substring(
