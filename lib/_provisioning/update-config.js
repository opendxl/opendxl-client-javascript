...
const ManagementService = require('../_provisioning/management-service')
const util = require('../util')
const provisionUtil = require('./provision-util')

const BROKER_CERT_CHAIN_COMMAND = 'DxlClientMgmt.createClientCaBundle'
const BROKER_LIST_COMMAND = 'DxlClientMgmt.getBrokerList'
...
function updateBrokerCertChain (configDir, certChainFile, certChain, verbosity) {
  const caBundleFileName = util.getConfigFilePath(configDir, certChainFile)
  if (verbosity) {
...
function brokerConfigLines (brokerListResponse) {
  const brokers = JSON.parse(brokerListResponse).brokers
...
function brokerWsConfigLines (brokerListResponse) {
  const brokersWs = JSON.parse(brokerListResponse).webSocketBrokers
...
function updateBrokerListInConfigFile (configFileName, brokerLines, verbosity, section) {
  ...
  let linesAfterBrokerSection = []
  const configFileData = util.getConfigFileData(configFileName)
  ...
  const newConfigLines = configFileData.lines.reduce(
    function (acc, line) {
      if (line === section) {
  ...
  let newConfig = newConfigLines.join(configFileData.lineSeparator)
  if (inBrokerSection) {
