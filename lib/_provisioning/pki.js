...
const provisionUtil = require('./provision-util')

const DEFAULT_PKI_FILE_PREFIX = 'client'

const DEFAULT_OPENSSL_WIN32_INSTALL_DIRS = ['C:\\OpenSSL-Win64\\bin',
  'C:\\OpenSSL-Win32\\bin']
const CSR_EXTENSIONS_CONFIG_SECTION = 'v3_req'
const PUBLIC_KEY_BITS = 2048
const SUBJECT_ALT_NAME_CONFIG_SECTION = 'alt_names'
...
module.exports = {
  ...
  runOpenSslCommand: function (description, commandArgs, opensslBin, verbosity,
    ...
    const command = childProcess.spawnSync(opensslBin, commandArgs,
      input ? { input } : {})
    ...
    if (command.stderr && ((command.status !== 0) || (verbosity > 1))) {
      provisionUtil.logError(indentedLogOutput(command.stderr),
        { header: 'OpenSSL ERROR LOG', verbosity })
    }

    if (command.status !== 0) {
      let errorMessage = 'openssl execution failed'
      if (command.error) {
  ...
  generatePrivateKeyAndCsr: function (configDir, commonOrCsrFileName, options) {
    const doneCallback = options.doneCallback
    const verbosity = options.verbosity || 0

    const filePrefix = options.filePrefix || DEFAULT_PKI_FILE_PREFIX
    const privateKeyFileName = path.join(configDir, filePrefix + '.key')
    let csrFileName = path.join(configDir, filePrefix + '.csr')
    ...
    const requestConfigAsString = requestConfigAsArr.join('\n')
    ...
    try {
      ...
      const commandArgs = ['req', '-new', '-newkey', 'rsa:' + PUBLIC_KEY_BITS,
        '-keyout', privateKeyFileName, '-out', csrFileName,
