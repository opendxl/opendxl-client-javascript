...
const DEFAULT_BROKER_CERT_CHAIN_FILE_NAME = 'ca-bundle.crt'
const PROVISION_COMMAND = 'DxlBrokerMgmt.generateOpenDXLClientProvisioningPackageCmd'
...
function brokersForConfig (brokerLines) {
  return brokerLines.reduce(function (acc, brokerLine) {
    const brokerElements = brokerLine.split('=')
...
function storeProvisionConfig (config, configDir, filePrefix,
  ...
  const configElements = config.split(',')
  ...
  const brokerLines = configElements[2].split(
    /[\r\n]+/).filter(function (brokerLine) {
    return brokerLine.length > 0
  }
  )

  const brokers = brokersForConfig(brokerLines)
  ...
  const brokersWS = brokersForConfig(brokerWSLines)
  const webSocketsEnabled = typeof useWebSockets !== 'undefined' ? useWebSockets : false
  const certFileName = filePrefix + '.crt'
  let configString = ini.encode({
    General: {
  ...
  const configFileName = path.join(configDir,
  ...
  const caBundleFileName = path.join(configDir,
  ...
  const fullCertFileName = path.join(configDir, certFileName)
  if (verbosity) {
...
module.exports = function provisionConfig (configDir, commonOrCsrFileName,
  ...
  options.doneCallback = function (error, privateKeyFileName, csrFileName) {
    if (error) {
      ...
      const service = new ManagementService(hostInfo)
