...
Client.prototype.asyncMultiServiceRequest = function (request, timeout, responseCallback) {
  ...
  var handler = new MultiServiceResponseHandler(request,
 ...
 * @constructor
 */
function MultiServiceResponseHandler (request, callback) {
 ...
 * @private
 */
MultiServiceResponseHandler.prototype.onResponse = function (response) {
  if (response.requestMessageId.startsWith(this._responseMessagePrefix)) {
    ...
    if (this._expectedRequestIds.filter(function (x) {
      return !this._receivedRequestIds.includes(x)
    }.bind(this)).length === 0) {
 ...
 */
MultiServiceResponseHandler.prototype.waitForResponses = function (initialResponse, timeout) {
  this._initialResponse = initialResponse

  var responsePayload = JSON.parse(initialResponse.payload.toString())
 ...
 * @private
 */
MultiServiceResponseHandler.prototype._returnResults = function () {
  if (this._callback) {
