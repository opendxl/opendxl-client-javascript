...
Message.prototype._packMessagev1 = function (buffer) {
  ...
  const otherFieldsAsArray = Object.keys(otherFields).reduce(
...
Message.prototype._unpackMessagev1 = function (raw) {
  const otherFields = {}
  const otherFieldsAsArray = this._unpackObject(raw)
  for (let index = 0; index < otherFieldsAsArray.length; index += 2) {
