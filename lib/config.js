...
const MalformedBrokerError = require('./malformed-broker-error')
const util = require('./util')
const provisionConfig = require('./_provisioning/provision-config')
const CustomProxy = require('./proxy') // Renamed to avoid redefinition issue

const DEFAULT_MQTT_KEEP_ALIVE_INTERVAL = 30 * 60 // seconds
const DEFAULT_RECONNECT_DELAY = 1 // seconds
 ...
 */
function getSetting (config, section, setting, required = true, readFromFile = false, configPath = null) {
  if (!config[section]) {
    if (required) {
      throw new DxlError('Required section not found in config: ' + section)
    } else {
      return null
    }
  }
  ...
  if (config[section][setting] === null || typeof config[section][setting] === 'undefined') {
    if (required) {
      throw new DxlError('Required setting not found in config: ' + setting)
    } else {
      return null
    }
  }

  let value = config[section][setting]

  if (readFromFile) {
    const fileToRead = util.getConfigFilePath(configPath, value)
    if (fs.existsSync(fileToRead)) {
      try {
        value = fs.readFileSync(fileToRead)
      } catch (err) {
        throw new DxlError('Unable to read file for ' + setting + ': ' + err.message)
      }
    } else {
      throw new DxlError('Unable to locate file for ' + setting + ': ' + value)
    }
  }
...
function parseBrokerString (parsedConfig, brokerList, section) {
  return Object.keys(brokerList).map(function (brokerKey) {
    const brokerInfo = getSetting(parsedConfig, section, brokerKey, true)
    if (typeof brokerInfo !== 'string') {
      throw new MalformedBrokerError('Config broker section has incomplete entries')
    }
    const brokerElements = brokerInfo.split(';')
    if (brokerElements.length < 2) {
      throw new MalformedBrokerError('Missing elements in config broker line: ' + brokerInfo)
    }
    ...
    const id = portPosition ? brokerElements[0] : null
    const port = brokerElements[portPosition]
    const hosts = brokerElements.slice(portPosition + 1)
 ...
 * @param {CustomProxy} proxy Information- If non null the proxy settings will be used. This is for
...
Config.createDxlConfigFromFile = function (configFile) {
  const parsedConfig = util.getConfigFileAsObject(configFile)
  const configPath = path.dirname(configFile)

  const brokerCaBundle = getSetting(parsedConfig, 'Certs', 'BrokerCertChain', true, true, configPath)
  const cert = getSetting(parsedConfig, 'Certs', 'CertFile', true, true, configPath)
  const privateKey = getSetting(parsedConfig, 'Certs', 'PrivateKey', true, true, configPath)

  // parse MQTT broker list
  let brokers = []
  if (Object.prototype.hasOwnProperty.call(parsedConfig, 'Brokers')) {
    brokers = parseBrokerString(parsedConfig, parsedConfig.Brokers, 'Brokers')
  }

  // parse WebSockets broker list
  let webSocketBrokers = []
  if (Object.prototype.hasOwnProperty.call(parsedConfig, 'BrokersWebSockets')) {
    webSocketBrokers = parseBrokerString(parsedConfig, parsedConfig.BrokersWebSockets, 'BrokersWebSockets')
  }

  let useWebSocketBrokers = getSetting(parsedConfig, 'General', 'UseWebSockets', false)

  // read proxy settings from config file if present
  const proxyAddress = getSetting(parsedConfig, 'Proxy', 'Address', false)
  let proxy = null
  if (proxyAddress) {
    const port = util.toPortNumber(getSetting(parsedConfig, 'Proxy', 'Port', true))
    const user = getSetting(parsedConfig, 'Proxy', 'User', false)
    let password
    if (user) {
      password = getSetting(parsedConfig, 'Proxy', 'Password', true)
    }
    proxy = new CustomProxy(proxyAddress, port, user, password)
  }
  ...
  const config = new Config(brokerCaBundle, cert, privateKey, brokers, webSocketBrokers, useWebSocketBrokers, proxy)
  const clientId = getSetting(parsedConfig, 'General', 'ClientId', false)
  if (clientId) {
 ...
 */
Config.provisionConfig = function (configDir, commonOrCsrFileName, hostInfo, options) {
