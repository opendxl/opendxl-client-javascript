...
CallbackManager.prototype.addCallback = function (messageType,
  topic, callback) {
  let callbacksByTopic = this._callbacksByMessageType[messageType]
  if (!callbacksByTopic) {
    callbacksByTopic = {}
    this._callbacksByMessageType[messageType] = callbacksByTopic
  }

  let callbacks = callbacksByTopic[topic]
  if (!callbacks) {
    callbacks = []
...
CallbackManager.prototype.removeCallback = function (messageType,
  topic, callback) {
  const callbacksByTopic = this._callbacksByMessageType[messageType]
  if (callbacksByTopic) {
    const callbacks = callbacksByTopic[topic]
    if (callbacks) {
      const callbackPosition = callbacks.indexOf(callback)
...
CallbackManager.prototype.onMessage = function (message) {
  const callbacksByTopic = this._callbacksByMessageType[message.messageType]
  if (callbacksByTopic) {
    ...
    const matchingTopicCallbacks = Object.keys(callbacksByTopic).reduce(
