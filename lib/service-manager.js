...
const DXL_SERVICE_UNAVAILABLE_ERROR_CODE = 0x80000001
const DXL_SERVICE_UNAVAILABLE_ERROR_MESSAGE =
...
function ServiceManager (client) {
  ...
  this._client.addRequestCallback('', function (request) {
    if (request.serviceId && !that._services[request.serviceId]) {
      const response = new ErrorResponse(request,
...
function registerService (client, service) {
  ...
  if (client.connected) {
    const request = new Request(DXL_SERVICE_REGISTER_REQUEST_TOPIC)
...
function unregisterService (client, service, callback) {
  if (client.connected) {
    const request = new Request(DXL_SERVICE_UNREGISTER_REQUEST_TOPIC)
    request.payload = JSON.stringify({ serviceGuid: service.info.serviceId })
...
ServiceManager.prototype.registerServiceAsync = function (
  ...
  serviceRegInfo.topics.forEach(function (topic) {
    service.callbacksByTopic[topic] = serviceRegInfo.callbacks(topic).map(
      function (callback) {
        const wrappedCallback = function (request) {
...
ServiceManager.prototype.unregisterServiceAsync = function (
  serviceRegInfo, callback) {
  const service = this._services[serviceRegInfo.serviceId]
  if (service) {
